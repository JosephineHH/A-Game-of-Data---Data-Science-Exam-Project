...
ggAcf(ts_data)
```
## checking for seasonlity across all data
```{r}
ts_data <- ts(data1$sentiment, frequency = 10)
#Fitting a linear model to our data
fit.ts_data <- tslm(ts_data ~ trend)
summary(fit.ts_data)

#Linear model with effect of season
fit.ts_season <- tslm(ts_data ~ trend + season)
summary(fit.ts_season)

#on the data from season
fit.ts_sent_season <- tslm(ts_data ~ season)
summary(fit.ts_sent_season)



AIC(fit.ts_data, fit.ts_season, fit.ts_sent_season)
