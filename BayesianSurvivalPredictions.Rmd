...
km_GOT <- with(GOT, Surv(time_of_monitoring, Dead))
head(km_GOT,80)
...
summary(km_fit_GOT, times = c(1*(1:10),67))
...
km_trt_fit_GOT <- survfit(Surv(time_of_monitoring, Dead) ~ Nobility, data=GOT)
autoplot(km_trt_fit_GOT)

#By gender
km_trt_fit_GOT <- survfit(Surv(time_of_monitoring, Dead) ~ Gender, data=GOT)
autoplot(km_trt_fit_GOT)

km_trt_fit_GOT <- survfit(Surv(time_of_monitoring, Dead) ~ Allegiances, data=GOT)
autoplot(km_trt_fit_GOT)

km_all_fit_GOT <- survfit(Surv(time_of_monitoring, Dead) ~ Gender + Nobility + Allegiances, data=GOT)
autoplot(km_all_fit_GOT)

#km_all_fit_GOT <- survfit(Surv(time_of_monitoring, Dead) ~ first_apperance, data=GOT)
#autoplot(km_all_fit_GOT)

survminer::ggsurvplot(km_fit_GOT,
           conf.int = TRUE,
           break.time.by = 20,
           risk.table = TRUE)
...
#Fit a cox-model
cox <- coxph(Surv(time_of_monitoring, Dead) ~ Nobility + Gender + Allegiances, data = GOT)
summary(cox, times = 10)
autoplot(cox)
...
```{r}
# ranger model
r_fit <- ranger(Surv(time_of_monitoring, Dead) ~ Nobility + Gender + Allegiances,
                     data = GOT,
                     mtry = 3,
...
#
cols <- colors()
for (n in sample(c(2:dim(GOT)[1]), 20)){
  lines(r_fit$unique.death.times, r_fit$survival[n,], type = "l", col = cols[n])
}
lines(death_times, avg_prob, lwd = 2)
legend(500, 0.7, legend = c('Average = black'))

#Most important variables according to ranger
...
#Plot the 3 models against each other
```{r}
# Set up for ggplot
kmi <- rep("KM",length(km_all_fit_GOT$time))
km_df <- data.frame(km_all_fit_GOT$time,km_all_fit_GOT$surv,kmi)
names(km_df) <- c("Time","Surv","Model")

coxi <- rep("Cox",length(cox$time))
cox_df <- data.frame(cox$time,cox$surv,coxi)
